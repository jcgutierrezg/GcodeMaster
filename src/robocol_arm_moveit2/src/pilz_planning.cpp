#include <iostream>
#include <memory>
#include <string.h>
// ROS libraries
#include <rclcpp/rclcpp.hpp>
#include <std_msgs/msg/string.hpp>
#include <std_msgs/msg/bool.hpp>
#include <std_msgs/msg/float32_multi_array.hpp>
#include <geometry_msgs/msg/pose.hpp>
// MoveIt libraries
#include <moveit/move_group_interface/move_group_interface.h>
using moveit::planning_interface::MoveGroupInterface;
#include <moveit_visual_tools/moveit_visual_tools.h>
#include <moveit/moveit_cpp/moveit_cpp.h>
// Placeholders
using std::placeholders::_1;

class PlanningNode : public rclcpp::Node {
  public :
    PlanningNode() : Node("planning_node") {
      // Create a ROS Subscription to robocol/arm_desired_pose with geometry_msgs::msg::Pose type message
      auto pose_topic = "robocol/arm/pose_data";
      RCLCPP_INFO(this->get_logger(), "Subscribing to %s topic...", pose_topic);
      pose_subscription_ = this->create_subscription<geometry_msgs::msg::Pose>(pose_topic, 1, std::bind(&PlanningNode::set_target_pose, this, _1));
      RCLCPP_INFO(this->get_logger(), "Subscribed to %s.", pose_topic);

      auto plan_topic = "robocol/arm/plan_exec";
      RCLCPP_INFO(this->get_logger(), "Subscribing to %s topic...", plan_topic);
      plan_subscription_ = this->create_subscription<std_msgs::msg::String>(plan_topic, 1, std::bind(&PlanningNode::set_action, this, _1));
      RCLCPP_INFO(this->get_logger(), "Subscribed to %s.", plan_topic);

      auto movetype_topic = "robocol/arm/movetype";
      RCLCPP_INFO(this->get_logger(), "Subscribing to %s topic...", movetype_topic);
      movetype_subscription_ = this->create_subscription<std_msgs::msg::String>(movetype_topic, 1, std::bind(&PlanningNode::set_movetype, this, _1));
      RCLCPP_INFO(this->get_logger(), "Subscribed to %s.", movetype_topic);

      auto velocity_accel_topic = "robocol/arm/velocity_accel";
      RCLCPP_INFO(this->get_logger(), "Subscribing to %s topic...", velocity_accel_topic);
      velocity_accel_subscription_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(velocity_accel_topic, 1, std::bind(&PlanningNode::set_velocity_accel, this, _1));
      RCLCPP_INFO(this->get_logger(), "Subscribed to %s.", velocity_accel_topic);

      planning_publisher_ = this->create_publisher<std_msgs::msg::Bool>("/robocol/arm/planning_info", 10);

      // Create move_group ROS node
      move_group_node = rclcpp::Node::make_shared("move_group");
      // Create a ROS logger
      auto const logger = rclcpp::get_logger("rclcpp logger");
      // Create the MoveIt MoveGroup Interface
      RCLCPP_INFO(logger, "Creating move group interface...");
      move_group_ptr_ = new MoveGroupInterface(move_group_node, "robocol_arm_group");
      RCLCPP_INFO(logger, "Move group interface created.");

      move_group_ptr_->setPlanningPipelineId("pilz_industrial_motion_planner");

      move_group_ptr_->setPlannerId("PTP");
      move_group_ptr_->setMaxVelocityScalingFactor(1.0);
      move_group_ptr_->setMaxAccelerationScalingFactor(1.0);
      move_group_ptr_->setPlanningTime(10.0);

      velocity = 0.3;
      accel = 0.3;
    }

    ~PlanningNode() {
      delete move_group_ptr_;
    }

  private :
    std::shared_ptr<rclcpp::Node> move_group_node;
    rclcpp::Subscription<geometry_msgs::msg::Pose>::SharedPtr pose_subscription_;
    rclcpp::Subscription<std_msgs::msg::String>::SharedPtr plan_subscription_;
    rclcpp::Subscription<std_msgs::msg::String>::SharedPtr movetype_subscription_;
    rclcpp::Subscription<std_msgs::msg::Float32MultiArray>::SharedPtr velocity_accel_subscription_;

    rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr planning_publisher_;

    geometry_msgs::msg::Pose target_pose;
    geometry_msgs::msg::Pose arc_center_pose;
    geometry_msgs::msg::Pose previous_planned_pose;

    bool circ_movement;
    bool center_flag;

    moveit::planning_interface::MoveGroupInterface *move_group_ptr_;
    moveit::planning_interface::MoveGroupInterface::Plan planned_path;
    bool ok;

    double velocity;
    double accel;


    void set_action(const std_msgs::msg::String::SharedPtr msg) {
      RCLCPP_INFO(this->get_logger(), "I heard: '%s'", msg->data.c_str());
      std::string plan_str = "plan";
      std::string execute_str = "execute";
      std::string planexec_str = "planexec";
      if (msg->data.c_str() == plan_str) {
        // Create a plan to that target pose
        RCLCPP_INFO(this->get_logger(), "Creating plan...");
        ok = static_cast<bool>(move_group_ptr_->plan(planned_path));
        if(ok) {
          previous_planned_pose = target_pose;

          auto flag = std_msgs::msg::Bool();

          flag.data = true;
          planning_publisher_->publish(flag);
        }
        else {

          auto flag = std_msgs::msg::Bool();
          flag.data = false;
          planning_publisher_->publish(flag);
        }
      } else if(msg->data.c_str() == execute_str) {
        // Execute the plan
        if(ok) {

          RCLCPP_INFO(this->get_logger(), "Executing plan...");
          move_group_ptr_->execute(planned_path);
          RCLCPP_INFO(this->get_logger(), "Plan executed.");
          //move_group_ptr_->setStartState(move_group_ptr->getCurrentState());
          move_group_ptr_->setStartStateToCurrentState();
        } else {
          RCLCPP_ERROR(this->get_logger(), "Can't execute because planing failed!");

        }
      } else if(msg->data.c_str() == planexec_str) {
        
        RCLCPP_INFO(this->get_logger(), "Creating plan...");
        ok = static_cast<bool>(move_group_ptr_->plan(planned_path));
        if(ok) {
          previous_planned_pose = target_pose;

          auto flag = std_msgs::msg::Bool();
          flag.data = true;
          planning_publisher_->publish(flag);
          
        // Execute the plan

          RCLCPP_INFO(this->get_logger(), "Executing plan...");
          move_group_ptr_->execute(planned_path);
          RCLCPP_INFO(this->get_logger(), "Plan executed.");
          //move_group_ptr_->setStartState(move_group_ptr->getCurrentState());
          move_group_ptr_->setStartStateToCurrentState();
        } else {

          auto flag = std_msgs::msg::Bool();
          flag.data = false;
          planning_publisher_->publish(flag);
          RCLCPP_ERROR(this->get_logger(), "Can't execute because planing failed!");
        }
      }
    }

    void set_movetype(const std_msgs::msg::String::SharedPtr msg) {
      RCLCPP_INFO(this->get_logger(), "I heard: '%s'", msg->data.c_str());
      
      std::string travel_str = "travel";
      std::string line_str = "line";
      std::string arc_str = "arc";

      if (msg->data.c_str() == travel_str) {
        // Create a plan to that target pose
        RCLCPP_INFO(this->get_logger(), "Changing planner to PTP");
        
        move_group_ptr_->setPlannerId("PTP");
        move_group_ptr_->setMaxVelocityScalingFactor(1.0);
        move_group_ptr_->setMaxAccelerationScalingFactor(1.0);

        circ_movement = false;

      } else if(msg->data.c_str() == line_str) {
        // Execute the plan
        RCLCPP_INFO(this->get_logger(), "Changing planner to LIN");
        
        move_group_ptr_->setPlannerId("LIN");
        move_group_ptr_->setMaxVelocityScalingFactor(velocity);
        move_group_ptr_->setMaxAccelerationScalingFactor(accel);

        circ_movement = false;

      } else if(msg->data.c_str() == arc_str) {
        // Execute the plan
        RCLCPP_INFO(this->get_logger(), "Changing planner to CIRC");
        
        move_group_ptr_->setPlannerId("CIRC");
        move_group_ptr_->setMaxVelocityScalingFactor(velocity);
        move_group_ptr_->setMaxAccelerationScalingFactor(accel);

        circ_movement = true;
        center_flag = false;
        
      }
      
    }

    void set_target_pose(const geometry_msgs::msg::Pose pose) {

      //RCLCPP_INFO(this->get_logger(), "I heard! %s", pose.orientation.x);

      if(!circ_movement) {

        target_pose = pose;
        RCLCPP_INFO(this->get_logger(), "Setting target pose...");
        move_group_ptr_->setPoseTarget(target_pose);
        RCLCPP_INFO(this->get_logger(), "Target pose set.");


      } else {

        if (!center_flag) {

          target_pose = pose;
          RCLCPP_INFO(this->get_logger(), "Setting target pose...");
          //move_group_ptr_->setPoseTarget(target_pose);
          RCLCPP_INFO(this->get_logger(), "Target pose set.");
          center_flag = true;

        } else {

          arc_center_pose = pose;

          moveit_msgs::msg::Constraints constraints;
          constraints.name = "center";  // Set a constraint name
          moveit_msgs::msg::PositionConstraint pos_constraint;
          pos_constraint.header.frame_id = "world";  // Frame ID of the center pose
          pos_constraint.link_name = "robocol_hand";  // Replace with your end-effector link
          pos_constraint.constraint_region.primitive_poses.push_back(arc_center_pose);
          //pos_constraint.target_point_offset.x = arc_center_pose.position.x;
          //pos_constraint.target_point_offset.y = arc_center_pose.position.y;
          //pos_constraint.target_point_offset.z = arc_center_pose.position.z;
          pos_constraint.weight = 1.0;  // Adjust the weight as needed
          constraints.position_constraints.push_back(pos_constraint);
          move_group_ptr_->setPathConstraints(constraints);
          RCLCPP_INFO(get_logger(), "Constraints Name: %s", constraints.name.c_str());

          move_group_ptr_->setPoseTarget(target_pose);

          center_flag = false;

        }

      }

    }

    void set_velocity_accel(const std_msgs::msg::Float32MultiArray::SharedPtr msg) {
      //RCLCPP_INFO(this->get_logger(), "I heard: '%s'", msg->data.c_str());
      
      velocity = msg->data[0];
      accel = msg->data[1];

      move_group_ptr_->setMaxVelocityScalingFactor(velocity);
      move_group_ptr_->setMaxAccelerationScalingFactor(accel);

      //RCLCPP_INFO(get_logger(), "Variable 1: %f", velocity);
      //RCLCPP_INFO(get_logger(), "Variable 2: %f", accel);
      
    }
};

int main(int argc, char * argv[]) {
  rclcpp::init(argc, argv);
  auto planning_node = std::make_shared<PlanningNode>();
  rclcpp::spin(planning_node);
  rclcpp::shutdown();
  return 0;
}
